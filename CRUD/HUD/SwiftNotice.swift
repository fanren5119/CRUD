...
//    }
//    
//    public func displayMessage() -> Void {
//        SwiftNotice.showNoticeWithText(.error, text: self.errorMessage(), autoClear: true, autoClearTime: 3)
//    }
//}
...
class SwiftNotice: NSObject {
    ...
    static func showNoticeWithText(_ type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) -> UIWindow {
        ...
        let size = text.boundingRect(with: CGSize(), options: NSStringDrawingOptions.usesFontLeading, attributes: [NSAttributedString.Key.font: font], context: nil);
        let width = min(max(size.width + 20, UIScreen.main.bounds.size.width / 2.5), UIScreen.main.bounds.size.width - 20)
        let frame = CGRect(x: 0, y: 0, width: width, height: 90)
        ...
        checkmarkView.frame = CGRect(x: (frame.size.width - 36) / 2, y: 15, width: 36, height: 36)
        mainView.addSubview(checkmarkView)
        
        let label = UILabel(frame: CGRect(x: 10, y: 60 + abs(size.origin.y), width: frame.size.width - 20, height: 16))
        label.font = font
        label.textColor = UIColor.white
        label.text = text
        label.textAlignment = NSTextAlignment.center
